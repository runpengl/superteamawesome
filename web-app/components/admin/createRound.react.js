...
module.exports = React.createClass({
  ...
  render: function() {
    ...
    return (
      <div>
        <h4>Add Rounds</h4>
        <form onSubmit={this._handleNewRoundSubmit}>
          <div className='form-element'>
            <label htmlFor='parent-round'>Parent Round</label>
            <select value={this.state.newRound.parentRound} onChange={this._handleParentRoundChange}>
          ...
          <div className='form-element'>
            ...
            <input defaultValue='' type='text' onChange={_this._handleRoundMetaLinkChange} />
          </div>
          <div className='form-element'>
          ...
          {this.state.newRound.names.map(function(name, index) {
            var deleteButton = '';
            if (index > 0) {
              deleteButton = <span className='delete' onClick={_this._handleRoundNameRemoval.bind(_this, index)}>-</span>
            }
            return (
              <div className='form-element' key={"add.round." + name.key} >
                <input defaultValue='' type='text' onChange={_this._handleRoundNameChange.bind(_this, index)} />
                {deleteButton}
              </div>
            );
          })}
          <div className='form-element'>
            <div className='extra-text' onClick={this._handleRoundNameAddition}>Add Another..</div>
          </div>
          <div className='form-element'>
