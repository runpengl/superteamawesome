...
function handleChromeRuntimeConnect(port) {
    ...
    switch (toolbarInfo.toolbarType) {
        ...
        case "puzzle":
            ...
            var unsubscribeFromSlack = subscribeToSlackChannel(
...
function authenticateSlack(userId) {
...
function handleSlackWsMessage(event) {
    ...
    switch (msg.type) {
        ...
        case "channel_marked":
            ...
            console.log("c_marked", channel.name, channel.unread_count_display);
            notifySubscribers(channel);
            break;
        case "message":
            var channel = slackChannelById[msg.channel];
            if (msg.user !== slackUserId && channel) {
                ...
                console.log("message", channel.name, channel.unread_count_display);
                notifySubscribers(channel);
            }
            break;
    }
...
function subscribeToSlackChannel(tabId, channelName, callback) {
